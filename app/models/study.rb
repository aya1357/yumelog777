class Study < ApplicationRecord
  ...
  def total_pages #読書をする総ページ数を返す(目次から索引までのページ数)
    end_number - start_number
  end

  def total_read_pages(user_id) #読書した総ページ数を返す
    logs.where(user_id: user_id).pluck(:study_number).sum
  end

  def remain_pages(user_id, studied_pages) #読書したページ数を加味して、残りページ数を返す
    remain_number = total_pages - (total_read_pages(user_id) + studied_pages.to_i)
    remain_number >= 0 ? remain_number : 0
  end

  def automatic_calculation_end_date(user_id, studied_pages) #読書したページ数を加味して、自動計算終了日として終了日を返す
    today = Date.today
    remaining_days = remain_pages(user_id, studied_pages)
    end_day = remaining_days == 0 ? today : get_end_date(today, remaining_days)
  end

  def get_end_date(today, remaining_days) #自動計算終了日を返す関数を設定
